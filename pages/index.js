...
export default function Home({ initialUser }) {
  ...
  const openCard = useCallback(async (catalyst) => {
    ...
    if (stockData[t]?.aiLoaded && stockData[t]?.aiV === 2) return
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetch(`/api/stockai?ticker=${t}&v=2`)
          .then(r => r.ok ? r.json() : null)
          .then(aiData => {
            console.log('AI data received:', JSON.stringify(aiData).slice(0,300))
            if (aiData && !aiData.noData) {
              setStockData(prev => ({ ...prev, [t]: { ...prev[t], ...aiData, aiLoading: false, aiLoaded: true, aiV: 2 } }))
            } else {
